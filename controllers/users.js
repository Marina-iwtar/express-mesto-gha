  ...
  User
    ...
    .catch((err) => {
      if (err.name === 'CastError') {
        next(new ErrorBadRequest('Переданы некорректные данные при выводе пользователя'));
      }
...
module.exports.editProfileUser = (req, res, next) => {
  ...
  User.findByIdAndUpdate(userId, { name, about }, { new: true, runValidators: true })
    ...
    .catch((err) => {
      if (err.name === 'ValidationError') {
        next(new ErrorBadRequest('Переданы некорректные данные при обновлении пользователя.'));
      }
      if (err.name === 'DocumentNotFoundError') {
        next(new ErrorNotFound('Пользователь с указанным _id не найден.'));
      }
